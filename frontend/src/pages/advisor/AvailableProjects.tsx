...
// --- 🌟 ฟังก์ชันยืนยันการอนุมัติ (เรียก API) ---
  const confirmApprove = async () => {
    ...
    try {
      // 1. ดึง Token จาก LocalStorage (ใช้ชื่อ key ให้ตรงกับในรูปภาพ)
      ...
      // 2. ถอดรหัส Token ดึงข้อมูลที่ซ่อนอยู่ออกมา
      const payload = JSON.parse(atob(token.split('.')[1]));
      console.log("🕵️‍♂️ ข้อมูลที่ซ่อนอยู่ใน Token คือ:", payload); // <-- เดี๋ยวเรามากด F12 ดูบรรทัดนี้กัน

      // 3. หาไอดีอาจารย์ (ต้องดูว่าใน Token คุณตั้งชื่อตัวแปรไว้ว่าอะไร)
      // ปกติมักจะเป็น id, advID, userId หรือ sub
      const advisorId = payload.advID || payload.id || payload.userId || payload.sub;

      if (!advisorId) {
        alert("หาไอดีอาจารย์ใน Token ไม่เจอ กรุณาเช็ก Console");
        return;
      }

      // 4. ส่งไปทั้ง 2 ค่า (Project ID และ Advisor ID)
      await projectService.approveProject(selectedProjectId, advisorId); 
  ...
  const filteredProjects = pendingProjects.filter((proj) => {
    ...
    const companyText = (proj.company?.coNameTH || proj.company?.coNameEN || proj.companyName || "").toString().toLowerCase();
  ...
  return (
    <div className="advisor-page">
      ...
      {showApproveModal && (
        <div className="advisor-modal-overlay">
          <div className="advisor-modal-content">
            ...
            <p>คุณแน่ใจหรือไม่ว่าต้องการ <strong>รับเป็นอาจารย์ที่ปรึกษา</strong> ให้กับโครงการนี้?<br/>เมื่ออนุมัติแล้ว โครงการจะเปิดให้นักศึกษาเข้าสมัครทันที</p>
            <div className="advisor-modal-actions">
              <button onClick={closeModals} className="btn-modal-cancel">
                ยกเลิก
              </button>
              <button onClick={confirmApprove} className="btn-modal-confirm-success">
                ยืนยันการอนุมัติ
              </button>
            </div>
          </div>
        ...
        <div className="advisor-modal-overlay">
          <div className="advisor-modal-content">
            ...
            <div className="advisor-modal-actions">
              ...
              <button onClick={confirmReject} className="btn-modal-confirm-danger">
                ยืนยันการปฏิเสธ
              </button>
            </div>
          </div>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <div className="action-buttons-group">
                      ...
                      <button
                        ...
                        onClick={() => handleOpenApproveModal(proj.projID || proj.id)}
                      ...
                      <button
                        ...
                        onClick={() => handleOpenRejectModal(proj.projID || proj.id)}
                        title="ปฏิเสธโครงการ"
                      >
      ...
      <div className="helper-note">
        <Info size={16} />
        <span>เมื่อกด <strong>"อนุมัติ"</strong> โครงการจะถูกเปลี่ยนสถานะเป็น APPROVED และคุณจะกลายเป็นอาจารย์ที่ปรึกษาทันที</span>
      </div>
    </div>
  );
}